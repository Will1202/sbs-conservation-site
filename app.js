...
/* Lightbox (shared: story + PV) */
let lbIdx = 0;
let lbSet = "story";

let lbSingle = [];
function getLightboxList(){
  if(lbSet === "single") return lbSingle;
  if(lbSet === "pv") return ASSETS?.pvImages || [];
  return ASSETS?.storyImages || [];
}
...
function openLightbox(setName="story", idx=0){
  const list = (setName === "pv") ? (ASSETS?.pvImages || []) : (ASSETS?.storyImages || []);
  if(!list.length) return;

  lbSet = setName;
  lbIdx = Math.max(0, Math.min(idx, list.length - 1));
  ...
  // restore navigation (non-single)
  const __prev = document.getElementById("lbPrev");
  const __next = document.getElementById("lbNext");
  const __note = document.querySelector(\'[data-i18n="lb_note"]\');
  if(__prev) __prev.style.display = "";
  if(__next) __next.style.display = "";
  if(__note) __note.style.display = "";
const item = list[lbIdx];
...
function openLightboxSingle({ url, zhTitle="", enTitle="", zhDesc="", enDesc="" }){
  lbSet = "single";
  lbIdx = 0;
  lbSingle = [{ url, zhTitle, enTitle, zhDesc, enDesc }];
  ...
  if(titleEl) titleEl.textContent = (lang==="zh") ? zhTitle : enTitle;
  if(descEl)  descEl.textContent  = (lang==="zh") ? zhDesc  : enDesc;

  const prev = document.getElementById("lbPrev");
  const next = document.getElementById("lbNext");
  const note = document.querySelector('[data-i18n="lb_note"]');
...
function setupLightbox(){
  document.getElementById("openLightboxBtn")?.addEventListener("click", ()=>openLightbox("story", 0));
...
function renderPVGallery(){
  ...
  const list = ASSETS?.pvImages || [];
  grid.innerHTML = "";

  list.forEach((it, i)=>{
    const div = document.createElement("div");
    div.className = "pvItem";
    div.addEventListener("click", ()=>openLightbox("pv", i));

    div.innerHTML = `
      <img src="${it.url}" alt="Tongzhou Bay image ${i+1}" />
      <div class="pvCap">${(lang==="zh") ? it.zhTitle : it.enTitle}</div>
    `;
    grid.appendChild(div);
  });
}

function setupCaseCard(){
  const img = document.getElementById("caseImg");
  const btn = document.getElementById("openCaseLightboxBtn");

  function openCase(){
    if(!img) return;
    openLightboxSingle({
      url: img.getAttribute("src"),
      zhTitle: "通州湾：空间规划图（节选）",
      enTitle: "Tongzhou Bay: planning map excerpt",
      zhDesc: "作为案例材料，用于呈现沿海开发与空间规划背景，并引出间接影响链条。",
      enDesc: "Case material framing coastal development/planning context and the indirect impact chain."
    });
  }

  if(btn) btn.addEventListener("click", openCase);
  if(img) img.addEventListener("click", openCase);
}
