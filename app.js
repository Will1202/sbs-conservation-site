...
function setupCaseCard(){
  ...
  const openCase = ()=>{
    ...
    openLightboxSingle(
      url,
      "通州湾：空间规划图（节选）",
      "Tongzhou Bay: planning map excerpt",
      "作为案例材料，用于呈现沿海开发与空间规划背景，并引出间接影响链条。",
      "Case material framing coastal development/planning context and the indirect impact chain."
    );
  };

  if(btn) btn.addEventListener("click", (e)=>{ e.preventDefault(); e.stopPropagation(); openCase(); }, true);
  if(img) img.addEventListener("click", (e)=>{ e.preventDefault(); e.stopPropagation(); openCase(); }, true);
}
